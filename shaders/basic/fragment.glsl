...
vec3 applyLinearFog(vec3 color) {
    ...
    vec3 fogColor = vec3(0.82, 0.55, 0.35);
 ...
 */
void main() {
    // Pre-compute normalized direction vectors for lighting calculations
    vec3 normalizedNormal = normalize(vNormal);           // Surface normal
    vec3 lightDirection = normalize(uLightPos - vPosition); // To light source
    vec3 viewDirection = normalize(uViewPos - vPosition);  // To camera
    ...
    if (uIsWater) {
        ...
        // Phong lighting: diffuse component (matte surface appearance)
        ...
        // Apply fog and render with slight transparency
        ...
        gl_FragColor = vec4(waterWithFog, 0.9);
        return;
    }
    ...
    // ────────────────────────────────────────────────────────────────────────
    // STANDARD PHONG LIGHTING (default for terrain, trees, etc.)
    // ────────────────────────────────────────────────────────────────────────
    // Choose base color: texture if enabled, otherwise uniform object color
    ...
    if (uUseGrassAnimation && !uIsWater) {  // Тільки на терені, не на воді
    ...
    // Phong lighting: ambient component (minimum illumination in shadows)
